...
/**
 * Server action to register a new user as a project manager.
 * Expects an object with `name`, `email`, `password`, and `role` fields.
 * Reads the API base URL from the environment variable `NEXT_PUBLIC_API_URL`.
 * Sends a POST request to `${apiUrl}/auth/register` and returns the parsed JSON.
